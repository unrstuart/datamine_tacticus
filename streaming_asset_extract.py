...
class UnityAssetExtractor:
    ...
    def extract_all_assets(self):
        ...
        try:
            ...
            for obj in env.objects:
                print(f"Processing object ID: {getattr(obj, 'path_id', 'unknown')}, Type: {getattr(obj.type, 'name', str(obj.type))}")
                try:
                    asset_count += 1
                    ...
                    if obj_type in ["MonoBehaviour", "Sprite", "TextAsset", "Texture2D"]:
        ...
        try:
            ...
            try:
                ...
                if obj_type == "MonoBehaviour":
                    print(f"MonoBehaviour read failed (ID: {obj.path_id}): {read_e}")
    ...
    def _extract_texture2d(self, data, obj, sequence_num):
        ...
        try:
            ...
            for attr in dir(data):
                if attr == 'image' or attr == 'set_object_reader':
                    continue
                print(f"  Texture2D attribute {attr}: {getattr(data, attr)}")
            if data.m_TextureFormat == 28 or data.m_TextureFormat == 29:
                ...
                uncompressed_size = data.m_CompleteImageSize
                #try:
                #    # Use .image property to handle *all* decompression (LZ4, Zlib, Crunch, DXT/PVRTC)
                #    # Save as PNG, as decompression converts to standard formats anyway
                #    filepath = self.texture2d_path / self._sanitize_filename(f"{name}.png")
                #    data.image.save(filepath) 
                #    print(f"✅ Extracted Texture2D via UnityPy: {filepath.name}")
                #except Exception as e:
                #    # If this crashes (Seg Fault) or fails, the native dependency is the issue.
                #    print(f"⚠️ UnityPy decompression failed: {e}")
                try:
                    decompressed_bytes = zlib.decompress(bytes, wbits=32)
                    ...
                    print(f"⚠️ Failed to decompress texture {name} using zlib. {e}")
                        
                try:
                    dctx = zstd.ZstdDecompressor()
                    decompressed_bytes = dctx.decompress(bytes)                    
                    decompressed_len = len(decompressed_bytes)
                    with open(filepath, 'wb') as f:
                        f.write(decompressed_bytes)
                    print(f"✅ Extracted Texture2D: {filename}")
                    ...
                    print(f"⚠️ Failed to decompress texture {name} using zstd. {e}")
                
                try:
                    # Decompress using raw LZ4 block and the known uncompressed size
                    ...
                    with open(filepath, 'wb') as f:
                        f.write(decompressed_bytes)
                    print(f"✅ Extracted Texture2D (LZ4 Block, {decompressed_len:,} bytes): {filename}")
                    return # Exit if successful
                except Exception as e:
                    print(f"⚠️ Failed to decompress texture {name} using lz4. {e}")

                try:
                    height = data.m_Height
                    width = data.m_Width
                    rgba = decode_crunch(bytes, width, height)
                    img = Image.frombytes('RGBA', (width, height), rgba)
                    img.save(filepath)
                except Exception as e:
                    print(f"⚠️ Failed to decode Crunch format for texture {name}. {e}")
                    rgba = None                
                    
                try:
                    height = data.m_Height
                    width = data.m_Width
                    rgba = decode_pvrtc(bytes, width, height)
                    img = Image.frombytes('RGBA', (width, height), rgba)
                    img.save(filepath)
                except Exception as e:
                    print(f"⚠️ Failed to decode Crunch format for texture {name}. {e}")
                ...
                print(f"❌ Failed to extract compressed Texture2D.")
                raw_len = len(bytes)
                if raw_len > 0:
                    print(f"🔬 Raw Stream Info: Compressed Size = {raw_len:,} bytes. Expected Decompressed Size = {uncompressed_size:,} bytes.")
                    print(f"🔬 Raw Stream Head (16 bytes): {bytes[:16].hex()}")
                    if raw_len > 16:
                        print(f"🔬 Raw Stream Tail (16 bytes): {bytes[-16:].hex()}")
                    else:
                        print(f"🔬 Raw Stream is very short (<= 16 bytes).")
                else:
                    print("🔬 Raw Stream is EMPTY.")
                return
            else:
                img = data.image
    ...
    def _extract_monobehaviour(self, env, data, obj, sequence_num):
        ...
        try:
            ...
            for attr in dir(data):
                print(f"  MonoBehaviour attribute {attr}: {getattr(data, attr)}")
            m_name_value = ''
...
def main():
    ...
    parser.add_argument("-o", "--output", default="streaming_assets",
                        help="Output folder (default: streaming_assets)")
    ...
    for file in args.input:
        if not os.path.exists(file):
            print(f"Error: Input path not found {file}")
            return 1

    for file in args.input:
        # Create extractor and execute
        extractor = UnityAssetExtractor(file, args.output)
